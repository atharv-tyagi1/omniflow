...
import { PageHeader, PageShell, SectionCard, GLASS_CARD_CLASSES } from "@/components/ui/dashboard-primitives"
    ...
    return (
      <PageShell variant="standard">
        <PageHeader 
          ...
          subtitle="Generate and schedule high-level operational reports."
        />
        <EmptyState 
          variant="coming-soon" 
    ...
    <PageShell variant="wide">
      ...
      <div className="grid gap-4">
        {plannedReports.map((report, idx) => (
          <div key={idx} className={cn(GLASS_CARD_CLASSES, "p-5 flex flex-col md:flex-row md:items-center justify-between gap-4")}>
            <div>
            ...
            <div className="flex-shrink-0">
              <span className="inline-flex items-center px-2.5 py-1.5 rounded-md text-xs font-medium bg-[var(--color-surface-hover)] text-[var(--color-text-secondary)] border border-[var(--color-border-subtle)]">
                {report.status}
              </span>
            </div>
          </div>
