...
import { Mail, Lock, User, Building, ArrowRight, Loader2, Command } from 'lucide-react';
...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await api.post<any>('/api/v1/auth/signup', { name, email, password, company_name: company });
      if (res && res.data) {
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-background relative overflow-hidden">
      ...
      <div className="z-10 w-full max-w-[460px] p-6 py-12">
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-3xl font-semibold tracking-tight text-primary-text">Create an account</h1>
          <p className="text-sm text-secondary-text mt-2">Get started with OmniFlow in seconds</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="glass-panel p-8 rounded-3xl space-y-5 relative overflow-hidden">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <User className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-text" />
                  <input
                    type="text"
                    value={name}
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-medium tracking-wide text-secondary-text uppercase">Company</label>
                <div className="relative">
                  <Building className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-text" />
                  <input
                    type="text"
                    value={company}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-medium tracking-wide text-secondary-text uppercase">Work Email</label>
              <div className="relative">
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-[10px] text-muted-text pl-1">Must be at least 8 characters</p>
            </div>
          </div>
        ...
        <p className="text-center text-sm text-secondary-text mt-8">
          ...
          <Link href="/login" className="text-accent-blue font-medium hover:text-accent-purple transition-colors">
            Sign in
          </Link>
        </p>
      </div>
