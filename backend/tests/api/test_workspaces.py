"""Tests for workspace management and isolation."""
...
async def test_workspace_isolation(async_client: AsyncClient):
    ...
    ws2_response = await async_client.get(
        "/api/v1/workspaces/current", headers=headers2
    )
    ws2_id = ws2_response.json()["data"]["id"]

    # User 1's current workspace is NOT User 2's workspace
    ws1_response = await async_client.get(
        "/api/v1/workspaces/current", headers=headers1
    )
