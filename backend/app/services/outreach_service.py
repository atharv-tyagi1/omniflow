...
class OutreachService:
    ...
    async def evaluate_triggers(db: AsyncSession):
        ...
        stmt = select(Ticket).where(
            and_(Ticket.status == "open", Ticket.created_at < stale_threshold)
        )
    ...
    async def _trigger_ticket_outreach(db: AsyncSession, ticket: Ticket):
        ...
        try:
            ...
            response_text = await client.generate_chat_response(
                [{"role": "user", "content": prompt}], "You are a customer care agent."
            )
    ...
    async def _trigger_lead_outreach(db: AsyncSession, convo: Conversation):
        ...
        try:
            response_text = await client.generate_chat_response(
                [{"role": "user", "content": prompt}], "You are a sales agent."
            )
