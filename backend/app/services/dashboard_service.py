...
class DashboardService:
    ...
    async def get_dashboard_metrics(db: AsyncSession, workspace_id: UUID) -> dict:
        ...
        total_conv_query = select(func.count(Conversation.id)).where(Conversation.workspace_id == workspace_id)
        total_conversations = await db.scalar(total_conv_query) or 0

        # 2. Tickets Analysis (Resolution Rate & Open Tickets)
        tickets_query = select(
            func.count(Ticket.id),
            func.sum(case((Ticket.status == 'resolved', 1), (Ticket.status == 'closed', 1), else_=0)),
            func.sum(case((Ticket.status == 'open', 1), (Ticket.status == 'in_progress', 1), else_=0))
        ...
        # Total tickets
        total_tickets_query = select(func.count(Ticket.id)).where(Ticket.workspace_id == workspace_id)
        ...
        sales_handoffs_query = select(func.count(func.distinct(Handoff.conversation_id))).join(
            Conversation, Handoff.conversation_id == Conversation.id
        ).where(
            Conversation.workspace_id == workspace_id,
            Handoff.to_agent == "sales"
        )
        sales_handoffs = await db.scalar(sales_handoffs_query) or 0
        
        lead_conversion = round((sales_handoffs / total_conversations * 100), 1) if total_conversations > 0 else 0.0

        # 4. Recent Activity (Last 5 conversations)
        recent_activity_query = select(Conversation).where(
            Conversation.workspace_id == workspace_id
        ...
        sentiment_query = select(Sentiment.label, func.count(Sentiment.id)).join(
            Conversation, Sentiment.conversation_id == Conversation.id
        ).where(Conversation.workspace_id == workspace_id).group_by(Sentiment.label)
        
        sentiment_result = await db.execute(sentiment_query)
        sentiment_distribution = [{"name": row[0].capitalize(), "value": row[1]} for row in sentiment_result.all()]
        
        # 6. Top Topics
        topics_query = select(Topic.topic_name, func.count(Topic.id)).join(
            Conversation, Topic.conversation_id == Conversation.id
        ).where(Conversation.workspace_id == workspace_id).group_by(Topic.topic_name).order_by(
