...
async def create_conversation(
    ...
    try:
        ...
        return SuccessResponse(data={"conversation_id": conversation.id}, message="Conversation created successfully")
...
async def list_conversations(
    ...
    conversations = await ConversationController.get_all(db=db, workspace_id=workspace_id)
    return SuccessResponse(data={"conversations": [c.id for c in conversations]}, message="Conversations retrieved")
...
async def analyze_conversation_intel(
    ...
    try:
        ...
        logging.getLogger(__name__).error(f"Failed to analyze conversation {conversation_id}: {e}")
...
async def get_conversation(
    ...
    try:
        ...
        return SuccessResponse(
            data={"conversation": {"id": conversation.id, "customer_id": conversation.customer_id, "channel": conversation.channel, "status": conversation.status}},
...
async def list_messages(
    ...
    try:
        ...
        return SuccessResponse(data={"messages": msg_list}, message="Messages retrieved successfully")
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
...
async def add_message(
    ...
    try:
        ...
        return SuccessResponse(data={"message_id": msg.id}, message="Message added successfully")
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
