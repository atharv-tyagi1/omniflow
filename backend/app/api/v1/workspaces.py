...
async def update_current_workspace(
    ...
    current_user: User = Depends(require_admin), # requires admin or owner to update settings
    db: AsyncSession = Depends(get_db)
):
    return await WorkspaceController.update_workspace(db, current_user.workspace_id, update_data)
